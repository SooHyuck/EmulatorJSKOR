    ...
    simulateInput(player, index, value) {
        ...
        if ([24, 25, 26, 27, 28, 29].includes(index)) {
            if (index === 24 && value === 1) {
                ...
                this.quickSave(slot);
                this.EJS.displayMessage(slot + " " + this.EJS.localization("슬롯에 상태를 저장했습니다."));
            }
            if (index === 25 && value === 1) {
                const slot = this.EJS.settings['save-state-slot'] ? this.EJS.settings['save-state-slot'] : "1";
                this.quickLoad(slot);
                this.EJS.displayMessage(slot + " " + this.EJS.localization("슬롯에서 상태를 불러왔습니다."));
            }
            if (index === 26 && value === 1) {
                let newSlot;
                try {
                ...
                this.EJS.displayMessage(newSlot + " " + this.EJS.localization("슬롯으로 상태 저장 슬롯을 변경했습니다."));
        ...
        try {
            if (fileNames.length > 1) {
                ...
                fileNames = fileNames.sort((a, b) => {
                    if (isNaN(a.charAt()) || isNaN(b.charAt())) throw new Error("파일 이름 형식이 잘못되었습니다.");
                    return (parseInt(a.charAt()) > parseInt(b.charAt())) ? 1 : -1;
                })
            }
        } catch(e) {
            if (fileNames.length > 1) {
                console.warn("CUE 파일을 자동 생성할 수 없습니다.");
                return null;
            }
        }
        for (let i = 0; i < fileNames.length; i++) {
            if (fileNames[i].split(".").pop().toLowerCase() === "ccd") {
                console.warn("CUE 파일을 자동으로 생성하지 못했습니다. CCD를 찾았습니다.");
                return null;
            }
        }
    ...
    loadPpssppAssets() {
        return new Promise(resolve => {
            this.EJS.downloadFile('cores/ppsspp-assets.zip', null, false, { responseType: "arraybuffer", method: "GET" }).then((res) => {
                this.EJS.checkCompression(new Uint8Array(res.data), this.EJS.localization("게임 데이터 압축을 해제합니다.")).then((pspassets) => {
                    if (pspassets === -1) {
                        this.EJS.textElem.innerText = this.localization('네트워크 오류가 발생했습니다.');
