(async function() {
    ...
    async function filesmissing(file) {
        ...
        if (minifiedFailed) {
            console.log("압축되지 않은 파일을 로드하려고 합니다.");
    ...
    if ((typeof window.EJS_language === "string" && window.EJS_language !== "ko-KO") || (systemLang && window.EJS_disableAutoLang !== false)) {
        const language = window.EJS_language || systemLang;
        try {
            let path;
            console.log("언어 로딩 중입니다.", language);
            ...
            console.log("언어가 누락되었습니다.", language, "!!");
